...
<div class="row">
            ...
            <div class="panel-body">
                ...
                @if(Auth::user()->tipo_id > 0)
                    ...
                    @foreach ($servicios_cliente as $servicio)
                        @if($servicio->id == 2)
                            {{ Form::open(array('route' => array('admin.carteleras.search'))) }}
                                @include('admin.carteleras.partials.formsearch')
    ...
    <div class="col-sm-12">
        <section class="panel">
            <header class="panel-heading">
                RESULTADOS
                <span class="badge bg-default">{{ $carteleras->count() }}</span>
            </header>
            <div class="panel-body">
            ...
            @if($carteleras->count())
                ...
                <tr class="">
                    ...
                    <td>
                        {{ date("d F Y",strtotime($cartelera->fecha_publicacion)) }}
                        <br>
                        @if(Entrust::can('editar_clientes'))
                        {{ User::where('id',$cartelera->user_id)->pluck('first_name') }} {{ User::where('id',$cartelera->user_id)->pluck('last_name') }}
                        @endif
                    </td>
                    <td>
                        <div class="botones">
                            ...
                            @if(Entrust::can('ver_cartelera'))
                                @if($cartelera->estado == 1)
                                <a class="btn btn-info btn-sm" href="{{ URL::route('admin.carteleras.show', $cartelera->id) }}" target="_blank"><i class="fa fa-columns"></i></a>
                                @else
