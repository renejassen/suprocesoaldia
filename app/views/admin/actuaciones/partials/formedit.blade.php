<div class="form-group {{ $errors->first('instancia_id', 'has-error') }}">
    ...
    <select id="instancia_id" name="instancia_id" class="populate" style="width:100%">
      ...
      @foreach($instancias as $instancia)
        @if($instancia->id === $actuacion->instancia_id)
        <option value="{{ $instancia->id }}" selected="">{{ Despacho::where('id',$instancia->despacho_id)->pluck('nombre') }}</option>
        @else
...
<div class="form-group {{ $errors->first('publicacion_id', 'has-error') }}">
    ...
    {{ Form::select('publicacion_id', $procesotipos = Publicacion::lists('tipo','id') , $actuacion->publicacion_id, array('id' => 'publicacion_id', 'class' => 'populate', 'style'  => 'width:100%')) }}
...
<div class="form-group {{ $errors->first('fecha_publicacion', 'has-error') }}">
  ...
  {{ Form::text('fecha_publicacion', $actuacion->fecha_publicacion, array('readonly', 'class' => 'form-control form-control-inline input-medium default-date-picker', 'size' => '36')) }}
...
<div class="form-group {{ $errors->first('fecha_auto', 'has-error') }}">
  ...
  {{ Form::text('fecha_auto', $actuacion->fecha_auto, array('readonly', 'class' => 'form-control form-control-inline input-medium default-date-picker', 'size' => '36')) }}
...
<div class="form-group {{ $errors->first('etapa_id', 'has-error') }}">
    ...
    {{ Form::select('etapa_id', $procesotipos = Etapa::lists('tipo','id') , $actuacion->etapa_id, array('id' => 'etapa_id', 'class' => 'populate', 'style'  => 'width:100%')) }}
...
<div class="form-group {{ $errors->first('actuacion_tipo_id', 'has-error') }}">
    ...
    {{ Form::select('actuacion_tipo_id', $procesotipos = ActuacionTipo::lists('tipo','id') , $actuacion->actuacion_tipo_id, array('id' => 'actuacion_tipo_id', 'class' => 'populate', 'style'  => 'width:100%')) }}
...
<div class="form-group {{ $errors->first('resumen', 'has-error') }}">
    ...
    {{ Form::textarea('resumen', $actuacion->resumen, array('class' => 'form-control', 'rows' => '6')) }}
...
<div class="form-group {{ $errors->first('fecha_audiencia', 'has-error') }}">
  ...
  {{ Form::text('fecha_audiencia', $actuacion->fecha_audiencia, array('readonly', 'class' => 'form_datetime form-control', 'size' => '36')) }}
